...
def main():
    ...
    for x in range(1, 25):
		...
		for ix, val_kilmer in ALL_ITEMS.items():
			...
			I = int(ix)
			img = val_kilmer['image_name']
			n = val_kilmer['n']
			ilvl = val_kilmer['ilvl']
			quality = val_kilmer['quality']
        	...
        	if 'consume' in val_kilmer.keys():
        		item.consume = True

        	if 'bop' in val_kilmer.keys():
        		item.bop = True

        	if 'unique' in val_kilmer.keys():
        		item.unique = True

        	if 'slot' in val_kilmer.keys():
        		item._slot = SLOT_TRANSLATOR[v['slot']]

        	if 'proficiency' in val_kilmer.keys():
        		item._proficiency = get_proficiency(item._slot, val_kilmer['proficiency'])

        	if 'quest_item' in val_kilmer.keys():
        		item.quest_item = True

        	# NOTE: need to change profession setup? axesmith, tribal lw, etc.
        	if 'requirements' in val_kilmer.keys():
        		item.requirements = val_kilmer['requirements']

        	# NOTE: need to exclude armor maybe?
        	if 'stats' in val_kilmer.keys():
        		item.stats = val_kilmer['stats']
