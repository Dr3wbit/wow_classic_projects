...
def javascript_REs():
	...
	# switching remaining 'x' values with 'maxRank' values, javascript flavor regex:
	r = re.compile(r"(x: \d+,\s+)(maxRank: \d+,\s+)")
	str.replace('$2$1')

	# above regex was in preparation for:
	# gets x coeff, replaces x and y functions with "fn: coeff * x"
	r = re.compile(r"x: (\d+),\s+(y: function\(\) \{\s+return )(this.x) \* this.invested\s+\}")
	str.replace("fn: ($1 * x)")

	# gets the simple standalone y functions and replaces them wit fn: (coeff * x)
	r = re.compile(r"(y: function\(\) \{\s+return )(\d+) \* this\.invested\s+\}")
	str.replace("fn: ($1 * x)")

	# same as above but with optional spaces (catches more)
	r = re.compile(r"(y: function\(\) \{\s+return )(\d+)\s?\*\s?this\.invested\s+\}")
	str.replace("fn: ($1 * x)")

	# wrapping functions in quotes to be JSON compliant
	r = re.compile(r"fn: ([\w\.\(\)\s\*\,\[\]]+?)(,\s+description)")
	str.replace("fn: "$1"$2")
