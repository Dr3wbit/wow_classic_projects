...
def main():
	...
	for ix in item_numbers:
		...
		if not error_box:
			...
			tabcont = driver.find_element(By.CSS_SELECTOR, "div.tabs-container")
			tabs = tabcont.find_elements(By.XPATH, "./div[@class='tabs-levels']/div[@class='tabs-level'][last()]/ul[@class='tabs']/li/a")

			selected = [x for x in tabs if x.get_attribute("class") is "selected"]

			num_tabs = len(tabs) # -2

			# NOTE: NEED TO FILTER OUT COMMENT AND SCREENSHOT TABS
			bad_i = [x for x,y in enumerate(tabs) if y.text.startswith(tuple(const.BAD_TABS))]
